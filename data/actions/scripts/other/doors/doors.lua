function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif table.contains(levelDoors, itemId) then
        ...
        elseif table.contains(keys, itemId) then
                if target.actionid > 0 and target:getAttribute(ITEM_ATTRIBUTE_UNIQUEID) == 0 then
                        if item.actionid == target.actionid and doors[target.itemid] then
                                target:transform(doors[target.itemid])
                                target:setAttribute(ITEM_ATTRIBUTE_UNIQUEID, 1)
                                return true
                        end
                elseif target.actionid > 0 and target:getAttribute(ITEM_ATTRIBUTE_UNIQUEID) == 1 then
                        if item.actionid == target.actionid and doors[target.itemid] then
                                target:transform(doors[target.itemid])
                                target:setAttribute(ITEM_ATTRIBUTE_UNIQUEID, 0)
                                return true
                        end
                        else
	...
	if table.contains(horizontalOpenDoors, itemId) or table.contains(verticalOpenDoors, itemId) then
        ...
        if doors[itemId] then
                if item.actionid == 0 or item:getAttribute(ITEM_ATTRIBUTE_UNIQUEID) == 1 then
